...
```
## [​](#lens-actions) Lens Actions
Each Nova lens also contains an `actions` method. This method allows you to attach any of your existing [actions](./../actions/defining-actions) to the lens:
app/Nova/Lenses/MostValuableUsers.php
Copy
Ask AI
```
...
## [​](#lens-metrics) Lens Metrics
Each Nova lens also contains a `cards` method. This method allows you to attach any of your existing [metrics](./../metrics/defining-metrics) to the lens:
Copy
Ask AI
```
...
```
## [​](#lens-pagination) Lens Pagination
If you would like to customize the selectable maximum result amounts shown on each lens’s “per page” filter menu, you can do so by customizing the lens’s `perPageOptions` property or `perPageOptions` method. By default, Nova will fetch the number of resources equal to the first value in the `perPageOptions` array:
Property
Method
Copy
Ask AI
