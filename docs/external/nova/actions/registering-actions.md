...
#footer > div > div:nth-of-type(n+2) {
    display: none;
}On this pageAuthorizationResource Specific AuthorizationAuthorization via Resource PolicyAction VisibilityInline ActionsStandalone ActionsSole ActionsPivot ActionsCustom Pivot Action NameClosure ActionsStatic ActionsRedirect ActionsVisit ActionsDanger ActionsCustom Modal ActionsOpen URLs in New TabsDownloading FilesAction Confirmation ModalFullscreen / Custom Modal SizesDisabling Action ConfirmationActionsRegistering ActionsRegistering actions in Nova.Once you have defined an action, you are ready to attach it to a resource. Each resource generated by Nova contains an actions method. To attach an action to a resource, you should simply add it to the array of actions returned by this method:
