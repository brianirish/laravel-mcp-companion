...
```
### [​](#queue-processing-during-maintenance-mode) Queue Processing During Maintenance Mode
When an environment is placed in maintenance mode, queue workers will stop processing jobs. Unlike traditional Laravel deployments where you can use `php artisan queue:work --force` to continue processing jobs during maintenance, Vapor does not currently support forcing queue processing during maintenance mode.
This means:
- All queued jobs will remain in the queue unprocessed while maintenance mode is active.
- Queue workers will automatically resume processing jobs once maintenance mode is disabled.
...
environments:
    production:
        ...
        build:
            - 'composer install --no-dev'
```
When configuring the memory for your Lambda function, you may define a value between 128 MB and 10,240 MB in 64-MB increments.Available values:
128, 192, 256, 320, 384, 448, 512, 576, 640, 704, 768, 832, 896, 960, 1024, 1088, 1152, 1216, 1280, 1344, 1408, 1472, 1536, 1600, 1664, 1728, 1792, 1856, 1920, 1984, 2048, 2112, 2176, 2240, 2304, 2368, 2432, 2496, 2560, 2624, 2688, 2752, 2816, 2880, 2944, 3008, 3072, 4096, 5120, 6144, 7168, 8192, 9216, and 10240
## [​](#concurrency) Concurrency
...
Vapor does not use standard Octane runtimes like Swoole, RoadRunner, or FrankenPHP. Instead, it runs on the AWS Lambda runtime environment. Because of this architectural difference, features that depend on Swoole or other specific runtimes may not be available or might behave differently than expected in Vapor deployments.
## [​](#gateway-versions) Gateway Versions
